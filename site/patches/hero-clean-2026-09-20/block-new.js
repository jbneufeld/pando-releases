    ...
    for (var n = 0; n < 34; n++){ var d = n * GAP + off, sc = P / (P + d), y = cy + floorY * sc; if (y > H + 2) continue;
      var a = 0.09 * hero * Math.pow(1 - n / 34, 1.6); ctx.strokeStyle = 'rgba(236,236,236,' + a.toFixed(3) + ')'; ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(W, y); ctx.stroke(); }
    /* verticals converging on the horizon */
    var xoff = ((cam.x % GAP) + GAP) % GAP, far = P / (P + 34 * GAP), yNear = cy + floorY, yFar = cy + floorY * far;
    for (var m = -14; m <= 14; m++){ var x = m * GAP - xoff, x0 = cx + x, x1 = cx + x * far; var a2 = 0.07 * hero * (1 - Math.min(1, Math.abs(m) / 14));
